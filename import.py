...
def get_reader(path, header=0, quoting=3):
    reader = pandas.read_csv(path, header=header, delimiter='	', encoding='latin1', low_memory=False,
                             quoting=quoting)
    reader.fillna(value='')
    return reader
...
def get_state_id(country, code):
    ...
    cursor.execute("select `id` from `state` where `country_id` = %s and (`name` = %s or `code` = %s)",
                   (country, code, code))
...
def get_object(table, where_column, where_value, company_specific=True):
    ...
    if company_specific:
        cursor.execute(f"select * from `{table}` where `company_id` = %s and `{where_column}` = %s",
                       (company, where_value))
    else:
...
def get_selling_unit_id(item_id):
    if item_id not in selling_unit_ids:
        cursor.execute(
            'select `selling_unit_id` from `style` inner join `item` on `style`.`id` = `item`.`style_id` where `item`.`id` = %s',
            item_id)
...
def get_mill_unit_id(item_id):
    if item_id not in mill_unit_ids:
        cursor.execute(
            'select `mill_unit_id` from `style` inner join `item` on `style`.`id` = `item`.`style_id` where `item`.`id` = %s',
            item_id)
...
def get_code_id(type_id, abbreviation, allow_unknown=False):
    ...
    if key not in get_ids:
        cursor.execute(
            "select `id` from `code` where `company_id` = %s and `type_id` = %s and (`abbreviation` = %s or `name` = %s)",
            (company, type_id, str(abbreviation), str(abbreviation)))
...
def get_custom_field_id(resource, name, field_type=1, default=None):
    cursor.execute("select `id` from `custom_field` where `company_id` = %s and `resource_class` = %s and `name` = %s",
...
def insert_address(source_id, source_type, street, city, country_id, name='Primary', street2=None, state_id=None,
                   postal_code=None, attention=None, company_name=None):
    address = {
...
def add_transaction_hold(transaction_id, hold_code):
    ...
    if hold_id is not None:
        try:
            insert_object('transaction_hold',
                          {'transaction_id': transaction_id, 'hold_id': hold_id, 'created_at': now, 'updated_at': now})
...
def import_vendor(data):
    ...
    cursor.execute("select * from `vendor` where `company_id` = %s and `name` = %s",
                   (company, str(data['VENDOR NAME'])))
    if cursor.fetchone() is not None:
    ...
    if data['CURRENCY^CODE'] == "EU" or data['CURRENCY^CODE'] == "DM" or data['CURRENCY^CODE'] == "DK" or data[
        'CURRENCY^CODE'] == "FF":
...
def import_style(data):
    ...
    cursor.execute("select * from `style` where `company_id` = %s and `name` = %s",
                   (company, str(data['PATTERN NAME'])))
    if cursor.fetchone() is not None:
...
def import_purchase_order_note(data):
    cursor.execute(
        "select `purchase_order_item`.`id` from `purchase_order_item` inner join `purchase_order` on `purchase_order_item`.`purchase_order_id` = `purchase_order`.`id` where `purchase_order`.`company_id` = %s and `purchase_order`.`purchase_order_number` = %s and `purchase_order_item`.`line_number` = %s",
        (company, re.sub(r'[^\d]', '', data['P.O.^NUMBER']), data['LINE^#']))
    row = cursor.fetchone()
    if row is not None:
        notes = ""
        for note in data[2:]:
            if isset(note):
                notes = f"{notes}\n{note}"
        if isset(notes):
            cursor.execute("update `purchase_order_item` set `internal_notes` = %s where `id` = %s",
                           (notes.strip(), row['id']))
...
def import_transaction(data, is_invoiced=False):
    ...
    transaction['shipping_service_id'] = get_column('customer', 'default_shipping_service_id', 'id',
    ...
    if is_invoiced:
        ...
        if transaction['discount_method'] == 'B' and 'discount_percent' in transaction:
            transaction['list_material_amount_invoiced'] = data['MATERIAL^AMOUNT'] / (
                        1 - transaction['discount_percent'])
        else:
            transaction['list_material_amount_invoiced'] = data['MATERIAL^AMOUNT']
...
def import_transaction_item(data, is_invoiced=False):
    ...
    if is_invoiced:
        ...
        if isset(data['P.O.NUMBER']):
            cursor.execute(
                "select `purchase_order_item`.`id` from `purchase_order_item` inner join `purchase_order` on `purchase_order_item`.`purchase_order_id` = `purchase_order`.`id` where `purchase_order`.`company_id` = %s and `purchase_order`.`purchase_order_number` = %s and `purchase_order_item`.`line_number` = %s",
                (company, re.sub(r'[^\d]', '', data['P.O.NUMBER']), data['PO^LINE']))
            row = cursor.fetchone()
            if row is not None:
...
def import_transaction_allocation(data, log=True):
    cursor.execute(
        "select `transaction_item`.`id`, `transaction_item`.`item_id`, `transaction`.`mill_cut_yardage` from `transaction` inner join `transaction_item` on `transaction`.`id` = `transaction_item`.`transaction_id` where `transaction`.`company_id` = %s and `transaction`.`legacy_transaction_number` = %s and `transaction_item`.`line_number` = %s",
        (company, data['ORDER #'], data['LIN^NUM']))
    ...
    if detail_record['mill_cut_yardage'] == 1:
        ...
        cursor.execute("select `id` from `inventory` where `item_id` = %s and `lot` = %s and `piece` = %s",
                       (detail_record['item_id'], data['LOT^NUMBER'], data['PCE^NUMBER']))
        inventory_record = cursor.fetchone()
        if inventory_record is None:
            if log:
                print(
                    f"Skipping transaction allocation for missing piece {data['ORDER #']}/{detail_record['item_id']}/{data['LOT^NUMBER']}/{data['PCE^NUMBER']}")
            return
...
def import_transaction_message(data):
    ...
    if data['ITEM NUMBER'] == 'Z':
        cursor.execute(
            "update `transaction` set `notification_message` = concat(ifnull(`notification_message`, ''), %s, '\n') where `id` = %s",
            (message, transaction_id))
        cursor.execute(
            "update `transaction` set `pick_ticket_message` = concat(ifnull(`pick_ticket_message`, ''), %s, '\n') where `id` = %s",
            (message, transaction_id))
        cursor.execute(
            "update `transaction` set `invoice_message` = concat(ifnull(`invoice_message`, ''), %s, '\n') where `id` = %s",
            (message, transaction_id))
...
cursor.execute(
    "delete `transaction_allocated_piece` from `transaction` inner join `transaction_item` on `transaction`.`id` = `transaction_item`.`transaction_id` inner join `transaction_allocated_piece` on `transaction_item`.`id` = `transaction_allocated_piece`.`transaction_item_id` where `transaction`.`company_id` = %s and `transaction`.`status` = 'C'",
    company)
cursor.execute(
    "delete `transaction_item` from `transaction` inner join `transaction_item` on `transaction`.`id` = `transaction_item`.`transaction_id` where `transaction`.`company_id` = %s and `transaction`.`status` = 'C'",
    company)
cursor.execute(
    "delete `transaction_service` from `transaction` inner join `transaction_service` on `transaction`.`id` = `transaction_service`.`transaction_id` where `transaction`.`company_id` = %s and `transaction`.`status` = 'C'",
    company)
...
if os.environ.get('IMPORT_VENDORS', 'false') == 'true':
    cursor.execute(
        ...
        (company))
    cursor.execute(
        "delete from `address` where `addressable_type` = 'Vendor' and `addressable_id` not in (select `id` from `vendor`)")
    cursor.execute(
        "delete `phone` from `phone` inner join `vendor` on `phone`.`phoneable_id` = `vendor`.`id` and `phone`.`phoneable_type` = 'Vendor' where `vendor`.`company_id` = %s",
        (company))
    cursor.execute(
...
if os.environ.get('IMPORT_CUSTOMERS', 'false') == 'true':
    cursor.execute(
        ...
        (company))
    cursor.execute(
        "delete ignore from `address` where `addressable_type` = 'Customer' and `addressable_id` not in (select `id` from `customer`)")
    cursor.execute(
        "delete `phone` from `phone` inner join `customer` on `phone`.`phoneable_id` = `customer`.`id` and `phone`.`phoneable_type` = 'Customer' where `customer`.`company_id` = %s",
        (company))
    cursor.execute(
...
if os.environ.get('IMPORT_REPS', 'false') == 'true':
    ...
    cursor.execute(
        ...
        (company))
    cursor.execute(
        "delete from `address` where `addressable_type` = 'Rep' and `addressable_id` not in (select `id` from `rep`)")
    cursor.execute(
        "delete `phone` from `phone` inner join `rep` on `phone`.`phoneable_id` = `rep`.`id` and `phone`.`phoneable_type` = 'Rep' where `rep`.`company_id` = %s",
        (company))
    cursor.execute(
...
if os.environ.get('IMPORT_CODES', 'false') == 'true':
    ...
    cursor.execute("update `code` set `at_proforma` = 1 where `company_id` = %s and `type_id` = 13 and `name` like %s",
                   (company, '%forma%'))
    cursor.execute(
        "update `code` set `at_proforma` = 0 where `company_id` = %s and `type_id` = 13 and `at_proforma` is null",
        company)
    cursor.execute(
        "update `code` set `pkg_length` = 36, `pkg_width` = 6, `pkg_height` = 6 where `company_id` = %s and `type_id` = 20",
        company)
    cursor.execute("update `line` set `default_package_code_id` = %s where `company_id` = %s",
...
if os.environ.get('IMPORT_WAREHOUSES', 'false') == 'true':
    ...
    cursor.execute(
        "update `line` set `default_warehouse_id` = (select `id` from `warehouse` where `company_id` = %s and `code` = %s limit 1) where `company_id` = %s",
        (company, default_warehouse_code, company))
    db.commit()
...
if os.environ.get('IMPORT_VENDORS', 'false') == 'true':
    ...
    for vendor_id, check_to_code in vendor_check_to.items():
        ...
        if check_vendor_id is not None:
            cursor.execute(f"update `vendor` set `send_check_vendor_id` = %s where `id` = %s",
                           (check_vendor_id, vendor_id))
    cursor.execute(
        f"insert into `vendor` (`company_id`, `legacy_vendor_code`, `name`, `country_id`, `currency_id`, `delivery_days`, `date_established`, `terms_id`) values ({company}, %s, %s, {country_us_id}, {currency_usd_id}, 0, curdate(), {code_unknown_terms_id})",
        ("UNK", "Unknown"))
...
if os.environ.get('IMPORT_REPS', 'false') == 'true':
    ...
    for index, rep in reps:
        import_rep(rep)
    cursor.execute(
        f"insert into `rep` (`company_id`, `rep_code`, `name`, `country_id`, `currency_id`, `start_date`) values ({company}, %s, %s, {country_us_id}, {currency_usd_id}, curdate())",
        ("UNK", "Unknown"))
...
if os.environ.get('IMPORT_STYLES', 'false') == 'true':
    ...
    for index, style in harmonized:
        ...
        if index % 500 == 0:
            db.commit()
    cursor.execute(
        "update `code` inner join `style` on `code`.`id` = `style`.`selling_unit_id` set `code`.`um_selling_unit` = 1 where `code`.`company_id` = %s",
        (company))
    cursor.execute(
        "update `code` inner join `style` on `code`.`id` = `style`.`mill_unit_id` set `code`.`um_mill_unit` = 1 where `code`.`company_id` = %s",
        (company))
    cursor.execute(
        "insert into `style` (`company_id`, `line_id`, `name`, `product_category_code_id`, `default_warehouse_id`, `selling_unit_id`, `mill_unit_id`, `vendor_id`, `standard_quantity`, `legacy_style_number`) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
        (company, line, 'Unknown', code_unknown_product_category_id, warehouse_def_id, code_unknown_unit_id,
         code_unknown_unit_id, unknown_vendor['id'], 1, 'UNK'))
...
if os.environ.get('IMPORT_INVENTORY', 'false') == 'true':
    ...
    for index, record in records:
        ...
        if index % 500 == 0:
            db.commit()
    cursor.execute(
        "update `inventory` inner join `item` on `inventory`.`item_id` = `item`.`id` inner join `style` on `item`.`style_id` = `style`.`id` set `inventory`.`vendor_id` = `style`.`vendor_id` where `inventory`.`company_id` = %s",
        company)
    cursor.execute(
        "update `inventory` inner join `vendor` on `inventory`.`vendor_id` = `vendor`.`id` inner join `warehouse` on `inventory`.`warehouse_id` = `warehouse`.`id` inner join `company` on `inventory`.`company_id` = `company`.`id` inner join `currency` as `vendor_currency` on `vendor`.`currency_id` = `vendor_currency`.`id` inner join `currency` as `warehouse_currency` on `warehouse`.`currency_id` = `warehouse_currency`.`id` inner join `currency` as `company_currency` on `company`.`currency_id` = `company_currency`.`id` set `exchange_rate_entry` = `vendor_currency`.`exchange_rate` / `warehouse_currency`.`exchange_rate`, `company_exchange_rate_entry` = `company_currency`.`exchange_rate` / `warehouse_currency`.`exchange_rate`, `inventory`.`currency_id` = `warehouse`.`currency_id` where `inventory`.`company_id` = %s",
        company)
    cursor.execute(
        "update `inventory` set `exchange_rate_receipt` = `exchange_rate_entry`, `company_exchange_rate_receipt` = `company_exchange_rate_entry` where `inventory`.`company_id` = %s",
        company)
    db.commit()
...
if os.environ.get('IMPORT_OPEN_TRANSACTIONS', 'false') == 'true' or os.environ.get('IMPORT_SHIPPED_TRANSACTIONS', 'false') == 'true':
    ...
    cursor.execute(
        ...
        company)
    cursor.execute(
        "update `transaction` inner join `customer` on `transaction`.`customer_id` = `customer`.`id` set `transaction`.`currency_id` = `customer`.`currency_id` where `transaction`.`company_id` = %s",
        company)
    cursor.execute(
        "update `transaction` inner join `currency` on `transaction`.`currency_id` = `currency`.`id` set `transaction`.`exchange_rate_entered` = 1 / `currency`.`exchange_rate`, `transaction`.`exchange_rate_invoiced` = 1 / `currency`.`exchange_rate` where `transaction`.`company_id` = %s and `transaction`.`status` = 'S'",
        company)
    cursor.execute(
        "update `transaction` inner join `currency` on `transaction`.`currency_id` = `currency`.`id` set `transaction`.`exchange_rate_entered` = 1 / `currency`.`exchange_rate` where `transaction`.`company_id` = %s and `transaction`.`status` = 'O'",
        company)
    cursor.execute(
        "update `transaction` inner join `transaction_hold` on `transaction`.`id` = `transaction_hold`.`transaction_id` set `transaction_hold`.`released_by_user_id` = %s, `transaction_hold`.`released_at` = now() where `transaction`.`company_id` = %s and `transaction`.`status` = 'S'",
        (user, company))
    cursor.execute(
        "update `transaction` left join (select `transaction_id`, max(`line_number`) as `max_line_number` from `transaction_item` group by `transaction_id`) as `items` on `transaction`.`id` = `items`.`transaction_id` left join (select `transaction_id`, max(`line_number`) as `max_line_number` from `transaction_service` group by `transaction_id`) as `services` on `transaction`.`id` = `services`.`transaction_id` set `transaction`.`next_line_number` = ifnull(greatest(`items`.`max_line_number`, `services`.`max_line_number`), 0) + 1 where `transaction`.`company_id` = %s",
        company)
    cursor.execute(
